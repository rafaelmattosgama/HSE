...
import type { FireEquipmentUrgentFloatingAlert as FireEquipmentUrgentFloatingAlertItem } from "@/lib/services/fire-equipment-alert-service";
...
/**
 * §8 exception: NON_CONFORMITY_FOUND can't wait for the next navigation — a
 * critical checklist item failing may mean equipment is unsafe to rely on —
 * so this polls every 30s like CompetenceUrgentAlert / SafetyCommunicationFloatingAlert
 * instead of going through the server-side RepeatabilityAlertModal.
 */
export function FireEquipmentUrgentAlert({
  plantCode,
  labels,
  enabled,
}: {
  ...
  const [alerts, setAlerts] = useState<FireEquipmentUrgentFloatingAlertItem[]>([]);
  ...
  useEffect(() => {
    ...
    async function loadAlerts() {
      try {
        const response = await fetch(`/api/plants/${plantCode}/notifications/fire-equipment`, {
          cache: "no-store",
        });
  ...
  // Above RepeatabilityAlertModal / SafetyCommunicationFloatingAlert (both z-[100]) but below
  // CompetenceUrgentAlert (z-[110], §7.1's own precedent for "someone may be operating equipment
  // without cover") — a failed critical fire-safety item is serious but competence suspension/
  // revocation was established first as this app's highest-priority overlay.
  return (
    <div className="fixed inset-0 z-[105] flex items-center justify-center bg-slate-950/45 px-4 backdrop-blur-[2px]">
                ...
                <div className="flex items-start gap-3">
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-3 flex flex-wrap items-center gap-3">
                      ...
                      >
                        {labels.urgentAlertOpenEquipment}
                      </Link>
                      <Button
                        type="button"
                        size="sm"
