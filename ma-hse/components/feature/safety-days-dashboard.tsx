...
function formatDate(dateKey: string | null) {
  if (!dateKey) return "No injury record";
  return dateKey;
}
        ...
        <div className="relative min-h-[280px] bg-[radial-gradient(circle_at_20%_20%,rgba(45,212,191,0.36),transparent_34%),radial-gradient(circle_at_84%_18%,rgba(251,191,36,0.34),transparent_26%),linear-gradient(135deg,#082f49_0%,#0f172a_48%,#3f1d49_100%)] p-6 sm:p-8">
          <div className="relative z-10 flex h-full flex-col justify-between gap-8">
            <div>
              ...
              <p className="mt-2 max-w-xl text-sm text-teal-50/85">
                Counter calculated from the latest injury record and the admin baseline date.
              </p>
            </div>
            ...
            <div>
              <div className="flex flex-wrap items-end gap-x-5 gap-y-2">
                ...
                <div className="pb-2">
                  <p className="text-lg font-bold text-teal-50">current days</p>
                  <p className="text-sm text-white/70">since {formatDate(summary.lastAccidentDate)}</p>
                </div>
              </div>
              <div className="mt-5 max-w-xl">
                ...
                <div className="mt-2 flex justify-between text-xs font-semibold text-white/75">
                  <span>Current</span>
                  <span>Record {summary.recordDays.toLocaleString()} days</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="grid content-center gap-4 bg-white p-5 text-slate-900 sm:p-6">
          <article className="rounded-xl border border-emerald-200 bg-emerald-50 p-4">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-xs font-semibold uppercase tracking-wide text-emerald-700">Current</p>
          ...
          <article className="rounded-xl border border-amber-200 bg-amber-50 p-4">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-xs font-semibold uppercase tracking-wide text-amber-700">Record</p>
                ...
                {summary.recordSource === "historical" ? (
                  <p className="mt-1 text-xs font-semibold text-amber-800">
                    Historical record{summary.historicalRecordStartDate ? ` since ${summary.historicalRecordStartDate}` : ""}
                  </p>
                ) : null}
              </div>
          ...
          <article className="rounded-xl border border-sky-200 bg-sky-50 p-4">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-xs font-semibold uppercase tracking-wide text-sky-700">To beat record</p>
...
export function GroupSafetyDaysBoard({ plants }: { plants: PlantSafetyDays[] }) {
      ...
      <div className="bg-[linear-gradient(135deg,#062c43_0%,#0f766e_48%,#f59e0b_100%)] p-6 text-white">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h2 className="mt-4 text-3xl font-black">Days without accidents across all plants</h2>
          </div>
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="rounded-xl border border-white/25 bg-white/15 px-4 py-3">
              <p className="text-xs font-semibold uppercase tracking-wide text-white/75">Best current</p>
            ...
            <div className="rounded-xl border border-white/25 bg-white/15 px-4 py-3">
              <p className="text-xs font-semibold uppercase tracking-wide text-white/75">Best record</p>
          ...
          <article key={plant.id} className="rounded-xl border border-slate-200 bg-slate-50 p-4">
            <div className="flex items-start justify-between gap-3">
              ...
              <span className="rounded-full bg-white px-3 py-1 text-xs font-semibold text-slate-600 shadow-sm">
                {formatDate(plant.safetyDays.lastAccidentDate)}
              </span>
            </div>
